...
def get_train_test_data_new(self, batch_size, file_path, time_step=60):
    ...
    if self_variable_name in self.file_path:
        cursor = self.file_path[self_variable_name]
    else:
        self.file_path[self_variable_name] = cursor = pd.read_csv(file_path, iterator=True)
    ...
    # 处理成矩阵形式返回
    return np.array(data_temp["0"].tolist()).reshape(batch_size, time_step * 7), np.array(
        data_temp["1"].tolist()).reshape(batch_size, 1)
...
def stock_lstm(self, ):
    ...
    # 获取当前python文件运行在那个目录下, 并去掉最后的文件名, 如: F:/deeplearning/main.py --> F:/deeplearning
    # 在linux下同样起作用
    basic_path = os.path.dirname(os.path.abspath(__file__))

    # 定义存储模型的文件路径
    # os.path.join(basic_path, "stock_rnn_save.ckpt")表示在运行的python文件路径下保存，文件名为stock_rnn.ckpt，
    # 在我环境下运行总是提示“另一个程序正在使用此文件，进程无法访问”，换到其他路径就OK
    model_save_path = "stock_rnn_save.ckpt"    #os.path.join(basic_path, "stock_rnn_save.ckpt")

    # 定义训练集的文件路径, 当前运行的python文件路径下, 文件名为 train_data.csv
    train_csv_path = os.path.join(basic_path, "train_data.csv")

    # 定义测试集的文件路径, 当前为运行的python文件路径下, 文件名 test_data.csv
    test_csv_path = os.path.join(basic_path, "train_data.csv")

    # 学习率
    learning_rate = 0.001

    # 喂数据给 LSTM 的原始数据有几行, 即: 一次希望LSTM能"看到"多少个交易日的数据
    origin_data_row = 60
    ...
    #每次给LSTM网络喂多少行进过处理的股票数据, 该参数依据自己显卡和网络大小动态调整, 越大一次处理的数据就越多, 越能占用更多的计算资源
    ...
    # None的意思就是喂数据时, 行数不确定, 给tf自动匹配
    # 我们喂的数据行数其实就是 batch_size , 但是因为None这个位置tf只接受数字变量, 而batch_size是placeholder定义的Tensor变量,
    # 表示我们在喂数据的时候才会告诉tf具体的值是多少
    input_x = tf.placeholder(tf.float32, [None, origin_data_col * origin_data_row])
    input_y = tf.placeholder(tf.float32, [None, output_num])

    # 处理过拟合问题。该值在其起作用的层上，给该层每一个神经元添加一个“开关”，“开关”打开的概率是keep_prob定义的值，一旦开关被关了，
    # 这个神经元的输出将被“阻断”。这样做可以平衡各个神经元起作用的重要性，杜绝某一个神经元“一家独大”，
    # 各种大佬都证明这种方法可以有效减弱过拟合的风险。
    ...
    # 转化成3维，是因为即将要进入LSTM层，接收3个维度的数据。粗糙点说，即LSTM接受：batch_size个，origin_data_row行cell_num列的矩阵，
    # 这里写-1的原因与input_x写None一致
    ...
    # 损失函数，用作指导tf
    # loss的定义为：(使用预测的收益 - 喂给LSTM的这60个交易日对应的真实收益)的平方，如果有多个（即：batch_size个且batch_size大于1），
    # 那就求一下平均值（先挨个求平方，再整个求平均值）
    ...
    with tf.Session(config=config) as sess:
        ...
        for i in range(100):
            try:
                # 读取训练集数据
                train_x, train_y = get_train_test_data_new(batch_size=_batch_size, file_path=train_csv_path)
            ...
            if (i + 1) % 20 == 0:
                ...
                print("保存模型\n")
                ############################################
                # 这部分代码作用为：每次保存模型，顺便将预测收益和真实收益输出保存至show_y_pre.txt文件下。熟悉tf可视化，
                # 完全可以采用可视化替代
        ...
        #读取测试集数据
        test_size = 100
        test_x, test_y = get_train_test_data_new(batch_size = test_size, file_path = test_csv_path)
